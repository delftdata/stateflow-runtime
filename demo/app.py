...
universalis.submit_protocol(g, user, order, stock)
...
def create_user():
    ...
    universalis.send_protocol_event(operator=user_operator,
                                    key=user_key,
...
def add_credit(user_key: str, amount: int):
    universalis.send_protocol_event(user_operator, user_key, user.AddCredit(), (user_key, amount))
...
def create_item():
    ...
    price: int = 1
    universalis.send_protocol_event(stock_operator, item_key, stock.CreateItem(), (item_key, item_name, price))
    return jsonify({'item_key': item_key})


@app.post('/stock/add_stock/<item_key>/<amount>')
def add_stock(item_key: str, amount: int):
    universalis.send_protocol_event(stock_operator, item_key, stock.AddStock(), (item_key, amount))
    return jsonify({'item_key': item_key})
...
def create_order(user_key: str):
    ...
    universalis.send_protocol_event(order_operator, order_key, order.CreateOrder(), (order_key, user_key))
...
def add_item(order_key: str, item_key: str):
    quantity, cost = 1, 1
    universalis.send_protocol_event(order_operator, order_key, order.AddItem(), (order_key, item_key, quantity, cost))
    return Response('Item added', status=200)


@app.post('/order/checkout/<order_key>')
def checkout_order(order_key: str):
    universalis.send_protocol_event(order_operator, order_key, order.Checkout(), (order_key,))
