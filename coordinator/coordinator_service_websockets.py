...
async def receive_data_coordinator(websocket, _):
    async for msg in websocket:
        deserialized_data: dict = cloudpickle.loads(msg)
        if '__COM_TYPE__' not in deserialized_data:
            ...
            if message_type == 'SEND_EXECUTION_GRAPH':
                ...
                await coordinator.submit_stateflow_graph_websockets(message)
            else:
                logging.error(f"COORDINATOR SERVER: Non supported message type: {message_type}")


async def main():
    async with serve(receive_data_coordinator, "0.0.0.0", SERVER_PORT):
